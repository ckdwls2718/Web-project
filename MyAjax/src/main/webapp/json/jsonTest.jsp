		...
		$("#bt1").on('click', function() {
			$.ajax({
				...
				success:function(res){
					//alert(res.title);
					let obj = JSON.parse(res); // json형식의 텍스트 문자열을 파싱해서 json 객체로 만들어서 반환해준다
					//JSON.parse(문자열) <==> JSON.stringify(객체)
					//alert(obj.publish);
					showData(obj);
				},
				error:function(err){
      ...
      즉, 아래에 있는 코드로는 수행이 안됨
		...
		$("#bt7").on('click', function(){
			...
			$.ajax({
				...
				url:url,
				headers:{
					'X-Naver-Client-Id':'hhrXhbpTWGADwolVK_Bh',
					'X-Naver-Client-Secret':'1qL2oWGYB2'
				},
				dataType:'json',
				cache:false,
			}).done(function(res){
		...
		$("#bt7").on('click', function(){
			...
			$.ajax({
				...
				$.each(books,function(i,book){
					...
					str+='<img src="'+book.image+'" style="width:20%">';
...
<div class="container">
	<h1>JSON 형태로 데이터를 받아봅시다</h1>
	<h2>JSON이란? - JavaScript Obejct Notation</h2>
	<h2>자바스크립트에서 이용하는 객체 형태로 데이터를 표현하는 방식</h2>
	<h3>JSON 객체에는 문자열, 숫자, 배열, boolean, null값만 들어갈 수 있다.</h3>
	<hr>
	...
	<br> <br>
	<button id="bt5" class="btn btn-warning">JSON형태로 받기5-라이브러리
		사용하기</button>
	<button id="bt6" class="btn btn-default">JSON형태로 받기6-라이브러리
		사용하기</button>
	<button id="bt7" class="btn btn-link">Naver OpenApi에서 도서정보
		받아오기</button>
	<hr>
